// ESE 680 - final projecy
// Understeer aware pure pursuit with RRT*
// Author: Xinlong Zheng, Samarth Kalluraya, Zixuan Lan
...
// converts grid index of map to xy coordinates in the world
...
// converts xy coordinates in the world to grid index of map 
...
// Generate the static map of the environment
...
void RRT::pf_callback(const geometry_msgs::PoseStamped::ConstPtr &pose_msg) {
  ...
  // Get position of car from particle filter
  ...
  //determine number of dynamic obstacles present within 3 meters of car
  ...
  //Update dynamic obstacles in map
  vector<double> point_pos_w;
...
// end of obstacle detection
  ...
  if(RRT_star){
    ...
    //find pure pursuit target (which will be the goal for rrt*)
    ...
    //initialize tree and the start node
    ...
    double radius = rrt_radius;      //radius for finding neighbourhood nodes
   ...
   // visualize tree
    ...
    //find new follow point on the rrt path
    double L_follow = global_L_follow;
    shortest = DBL_MAX;
    //backtrack to find the actual rrt* path
  ...
  if(!RRT_star){
    ...
    //this for loop finds the closest waypoint, the waypoint for purse pursuit 
    //and waypoints where we will check future belocities
